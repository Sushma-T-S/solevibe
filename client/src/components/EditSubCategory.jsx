...
const EditSubCategory = ({close,data,fetchData}) => {
    ...
    const handleSubmitSubCategory = async(e)=>{
        ...
        try {
            ...
            const categoryIds = subCategoryData.category.map(cat => {
    ...
    <section className='fixed top-0 right-0 bottom-0 left-0 bg-neutral-800 bg-opacity-70 z-50 flex items-center justify-center p-4'>
        <div className='w-full max-w-5xl bg-white p-5 rounded-2xl shadow-lg border border-slate-200'>
            <div className='flex items-center justify-between gap-3'>
                <h1 className='font-bold text-slate-900'>Edit Sub Category</h1>
            ...
            <form className='my-3 grid gap-3' onSubmit={handleSubmitSubCategory}>
                    ...
                    <div className='grid gap-1'>
                        ...
                        <div className='flex gap-4 flex-col md:flex-row md:items-center'>
                            ...
                            <label htmlFor='uploadSubCategoryImageEdit'>
                        ...
                        <div className={`border rounded-xl ${errors.category ? 'border-red-400' : 'border-slate-300'} focus-within:border-indigo-600 focus-within:ring-4 focus-within:ring-indigo-100`}>
                            <div className='flex flex-wrap gap-2'>
                                {
                                    subCategoryData.category.map((cat,index)=>{
                                        return(
                                            <p key={(cat._id || cat)+"selectedValue"} className='bg-slate-50 border border-slate-200 rounded-full px-3 py-1 m-1 flex items-center gap-2 text-sm text-slate-800'>
                                                {cat?.name || cat}
                                                <div className='cursor-pointer hover:text-red-600' onClick={()=>handleRemoveCategorySelected(cat?._id || cat)}>
                                                    <IoClose size={20}/>
                                                </div>
                                            </p>
                                        )
                            ...
                            <select
                                ...
                                onChange={(e)=>{
                                    ...
                                    setSubCategoryData((preve)=>{
                                        if(preve.category.some((c)=> (typeof c === 'string' ? c === categoryDetails._id : c?._id === categoryDetails._id))){
                                            return preve
                                        }
                                        return{
                                            ...preve,
